...
(הידע שלי דל בנושא לומד תוך כדי כתיבה) 

 לכל סוג של מעבד יש סט פקודות שונה אנחנו נתמקד בפקודות של מעבדים בתצורת x86 

כשהמפעילים מחשב הוא יש מערכת בשם BIOS שקוראת את הפקודות השמורות בדיסק ומתחיל לבצע לפי הסדר, בימנו ברוב המחשבים הגדולים זה יהיה פקודות של המערכת הפעלה אבל אפשר לכתוב תוכנה שתעבוד ישירות על המחשב בלי מערכת הפעלה (למשל בבקרים).

כל פקודה זה בעצם פעולה שהמעבד יכול לבצע ומסומנת עם 01 למשל פקודת MOV כתובה כך (ממציא משהו עד שאני אמצא את הקוד הנכון) 00110001
אבל בגלל שלא נוח לקרוא 0011001 אז משתמשים בשפת אסמבלי שממירה משפה קצת יותר אנושית לשפה מכונה ולהיפך.

עכשיו קצת סקריה על הפקודות השונות  
הפקודה MOV מעבירה מידע ממקום   
והפקודה ADD מבצעת פועלת חיבור של שני ערכים
למשל
```
MOV RO,#1
MOV R1,#2
ADD R2,R0,R1
```
כאן אנחנו כותבים לR0 (רג'יסטר מספר 0) את הספרה 1, ולR1 את הספרה 2,  
ואז מבצעים פעולה חיבור שאת התוצאה נכתוב לR2 (ואז אפשר לבצע עוד פעולות עם הערך שנשמר בR3 או לשלוח אותו לפלט של המחשב שזה סיפור בפני עצמו) 

## שפות עיליות
מכיוון שזה מורכב לכתוב תוכנות באסמבלי המציאו שפות שיותר קלות לקריאה לבני אדם
ובאמצעות תוכנה ממירים את הפקודות לשפת מכונה, אחרי ההמרה קשה לשחזר את הקוד המקורי מתוך הקוד בשפת מכונה אבל יש כלים שעוזרים לעשות את זה באופן חלקי וכך לפחות להבין מה התוכנה עושה.
...
## מערכות הפעלה
כיוון שאנחנו רוצים להריץ משפר תוכנות על אותו מחשב יש צורך בתוכנה שתריץ אותם והתוכנה הזאת מכונה מערכת הפעלה
